...
class SparseMatrixMaker:
    ...
    to the disk.
    """
    def __init__(self, n_last_years: int = 5, min_marks: int = 20, marks_df_path='data/raw/marks.csv.gz'):
        self.n_last_years = n_last_years
        self.min_marks = min_marks
        self.marks_df = pd.read_csv(marks_df_path)
    ...
    def make_sparse_matrix(self):
        ...
        user_dict = dict(enumerate(self.marks_df.user_id.unique()))
        user_dict = {value: key for key, value in user_dict.items()}

        work_dict = dict(enumerate(self.marks_df.work_id.unique()))
